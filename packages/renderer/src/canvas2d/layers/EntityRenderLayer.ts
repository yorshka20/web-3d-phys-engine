...
export class EntityRenderLayer extends CanvasRenderLayer {
  constructor(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) {
    super(
      RenderLayerIdentifier.ENTITY,
      RenderLayerPriority.ENTITY,
      canvas,
      context
    );
  }
  ...
  renderEntity(
    ...
    if (entity.hasComponent(AnimationComponent.componentName)) {
      const animation = entity.getComponent<AnimationComponent>(
        AnimationComponent.componentName
      );
   ...
   */
  private renderAnimatedEffectEntity(
    animation: AnimationComponent,
    shape: ShapeComponent
  ): void {
    ...
    const totalColumns = Math.max(
      1,
      Math.floor(spriteSheet.image.width / frameWidth)
    );
  ...
  private renderPlayerEnemyEntity(
    ...
    if (state.getIsHit() && spriteSheet.animations.has("hurt")) {
      ...
      !state.getIsHit() &&
      animation.getCurrentAnimation() === "hurt"
    ) {
      ...
      if (
        spriteSheet.animations.has("walk") &&
        animation.getCurrentAnimation() !== "walk"
      ) {
        animation.setAnimation("walk");
      }
    }
