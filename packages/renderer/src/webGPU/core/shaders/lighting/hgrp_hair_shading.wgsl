// The CharacterNPR_Hair shading, shared by the hair material (materials/HGRPHair.wgsl) and the
// under-brow draw of the strands the brow shows through (passes/hgrp_hair_under_brow.wgsl):
// shade blend under the NPR lighting multiplier through the diffuse half of _SplitNormalMap,
// and the hair shader's Kajiya-Kay specular (hgrp-decompiled-formulas.md §3): three lobes
// around a strand tangent shifted along the specular normal — the primary band colored by the
// spec ramp, a secondary band in _AnisotropyColor2 where the primary is weak, and a third lobe
// that gates the darkening and desaturation of the strand lines. Every lobe fades with the
// horizontal n.v in object space. The surface map's G is the specular scalar (Pelica's
// _Specular is 0 and its map lights only the bangs cards — the in-game single band), its R
// selects the strand direction (0 = the object-space up projected onto the strand plane,
// 1 = the mesh bitangent) and its A scales the secondary lobe.
...
fn hgrp_shade_hair(input: GLTFVertexOutput) -> vec4<f32> {
