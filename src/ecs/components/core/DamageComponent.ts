...
export class DamageComponent extends Component {
  ...
  constructor(props: DamageProps) {
    ...
    this.tickRate = props.tickRate ?? 100;
  ...
  isAoe(): boolean {
    return (
      this.weapon?.type === WeaponType.AREA ||
      this.weapon?.type === WeaponType.BOMB
    );
  }
  ...
  isLaser(): boolean {
    return (
      this.weapon?.type === WeaponType.LASER ||
      this.weapon?.type === WeaponType.LASER_BURST
    );
  }
