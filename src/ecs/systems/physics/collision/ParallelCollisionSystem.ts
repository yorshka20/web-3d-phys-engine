...
export class ParallelCollisionSystem extends System {
  ...
  private async handleWorkerResults(
    activePromises: Promise<CollisionPair[]>[]
  ) {
    try {
        ...
        for (let i = 0; i < this.positionalCorrectTimes; i++) {
          ...
          for (const pair of uniqueCollisions) {
            ...
            if (
              ...
              if (physicsA?.isAsleep() || physicsB?.isAsleep()) {
                this.wakeUpClusterForEntity(
                  physicsA.isAsleep() ? entityA.id : entityB.id
                );
              }
  ...
  private checkObjectObjectCollision(
    ...
    const shapeA = entityA.getComponent<ShapeComponent>(
      ShapeComponent.componentName
    );
    const shapeB = entityB.getComponent<ShapeComponent>(
      ShapeComponent.componentName
    );
    ...
    return getCollisionNormalAndPenetration(
      posA,
      sizeA,
      typeA,
      posB,
  ...
  private filterUniqueCollisions(
    allCollisions: CollisionPair[]
  ): Set<CollisionPair> {
